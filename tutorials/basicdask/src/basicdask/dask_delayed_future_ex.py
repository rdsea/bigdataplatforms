...
# datatype
dtype = {"station_id": str, "alarm_id": int, "parameter_id": int}
...
if __name__ == "__main__":
    ...
    bts_alarm_df = dd.read_csv(
        input_file_alarm, dtype=dtype, assume_missing=True, low_memory=False
    )
    # simple filtering of input data
    alarm_filter = (bts_alarm_df["isActive"] is True) & (
        bts_alarm_df["alarm_id"] == HIGH_ROOM_TEMP_ID
    )
    bts_alarm_df = bts_alarm_df[alarm_filter]
    bts_parameter_df = dd.read_csv(
        input_file_param, dtype=dtype, assume_missing=True, low_memory=False
    )
    bts_parameter_df = bts_parameter_df[
        bts_parameter_df["parameter_id"] == OUTDOOR_TEMP_ID
    ]
