...
Working with kafka-shell is quite cumbersome. So, we can instead use Kafkacat to work with Kafka. [Kafkacat](https://github.com/edenhill/kafkacat) is a popular non JVM utility that allows producing, consuming and listening to Kafka. Instead of writing long commands and code, we can use it to learn kafka very quickly.
...
We have two simple programs: [a simple producer](code/simple_kafka_producer.py) and [a simple consumer](code/simple_kafka_consumer.py) that you can play with by using the [ONU data](/data/onudata/README.md).
